...
// GYM OF TUCSON MAP
// Declare the marker globally

async function initMap() {
  const { Map } = await google.maps.importLibrary("maps");
  const { AdvancedMarkerElement } = await google.maps.importLibrary("marker");

  // The location of The Gym of Tucson
  const gymLocation = { lat: 32.338732409823805, lng: -111.05388367055221 };

  // The map, centered at The Gym of Tucson
  const map = new Map(document.getElementById("map"), {
      zoom: 17, // Adjust the zoom level as you like.
      center: gymLocation,
      ...
      mapId: "e594d96c95496f97" // Map ID is required
  });

  // Setting global font style, unfortunately, Google Maps doesn't allow changing font family
  document.getElementsByTagName('body')[0].style.fontFamily = 'Arimo, sans-serif';
